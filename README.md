...
| `scripts/` | `run_gold.py` 金标准评测 · `fq_shadow.py` 特征层影子跑（与现行抽取器 / D-081 表比对，出 SQL）· `backfill_comments.py` 评论回填 · `external_corpus.py` 外部语料定时抓取（`--dry-run` 算钱、`--probe` 钉字段）· `apply_sql.py` |
...
| `tests/` | 22 个测试，全 mock（评论回路用按关键词给答案的假 Jev），不联网 |
...
uvicorn judge.api:app --port 8080              # HTTP：POST /judge · GET /banks · GET /health（X-Judge-Key）
...
环境变量：`TYPESAFE_API_KEY`（必需）· `JEV_MODEL`（可覆盖题库里的版本，默认不用）· `JUDGE_API_KEY`（HTTP 鉴权）·
`SUPABASE_URL` + `SUPABASE_SERVICE_KEY`（直接写账本；没有就只出 SQL）· `JUDGE_MOCK=1`（不联网）· `TIKHUB_API_KEY`（外部语料，每次请求 0.01 美元）。
...
去重靠 `state/external_corpus_state.json`（GitHub Actions 用 cache 保留）和 `--known-ids`（账本里已有的 external note_id）。
