...
> **第五轮（2026-09-24，「剩下的全部」：§2、TV 答题契约 a–h、§4、§5、§6）**。本仓：
> §2 #1 闸二门禁：没过闸二的 fq 目标题不进修改单、只进 `recorded`，fq 硬伤只给依据句不给题干定义，按题库格式认 fq（换名加载也守得住）；#2 暗题：`judge/hidden.py` + `config/hidden_rotation.yaml`，人感题库每季度按 sha256(题号|季度) 自动换 1/3，修改单、`/judge_draft`、`/banks`、MCP 全不回暗题；#3 写手侧 MCP 与 HTTP 共用 `judge/draft.py`，MCP 能送 brief / hard_rules，`repair_plan_for` 带项目层；#4 数据出境：`judge/policy.py` + `config/data_policy.yaml`，未发布稿必须带 project，处方药（OKMAN）未清出境 403、一次 Jev 都不调，项目层只有放行的项目才跑、否则整层去掉并回显，预埋评论可用 `published: false` 收紧；#5 评论区没有摩擦也算不过并换位（有贴主回复位换它，没有换提问位，都没有记为配置问题不换），`一半左右` 放行写明；#6 已在第四轮修；#7 「蓝词植入」留在闭集（TV CHECK 逐字相同），co-v0.2 定义改成只看文字能判的点名植入，state 不带蓝词清单；#8 `thread_cases.json` 删掉「帖子数据」，加一条扫全部题库中文文本与各类 state 的 PERFORMANCE_KEYWORDS 测试（t1 的金标准要有 Jev 密钥才能重跑，本环境没有，留给 owner）；#9 `human_feel_para_v0.2` 的 para_function 加「说不清」、`comment_thread_v0.4` 的 praise_share 加「评论太少」，旧版保留；#10 529 退避重试。
> TV 契约：a 跳题逐字照 TV 的 render_call（full 看标题 + 正文，1 字标题也算太短）；b 全跳零调用；c 需要证据的答案没证据一律 evidence_not_found，`write=true` 不许 `with_evidence=false`；d mock 判出的行 extractor 恒为 `mock:<模型>`，API 与写库函数都拒绝写；e 证据存整句是有意的差异，写进 core.py 顶部；f 问了没返回记 missing、答案不在选项里或没有概率记 out_of_vocab；g choice 题证据规则按 TV 的「不需要 / 选『X』以外」通用解析，逐题与 TV 的 evidence_required 对拍；h 无效行 prob 为 NULL。
> §6：fq 题库名不再写死；MCP 不再用排除法猜项目题库；换位原因写进这一位的 note 交给 prompt_for，换谁都修不了的不换；回复位不再重出两次；漏答不当硬伤、整条进复核；`removeprefix`；EchoGenerator 真的按依据句删句、测试覆盖修补；正文太短整篇记 text_too_short、不算过；证据调用与运营题库的用量计入；生成端剥尾部 /v1、读 MOONSHOT_API_KEY、退避重试；MCP 共用一个客户端；「0」不再取到最后一句；切句上限 254（Jev 选项上限）；PostgREST 写入刷新 extracted_at；mock 答案与题目插入顺序无关；`items[].note` 包装与非 dict 元素；周任务加 `known_external_ids.py` 从库里拉已入库 id 做第二道去重。另外：没判出有效答案的硬约束题记进 `unjudged`、不算过；vendor 的 TV 题库与 feature_bank.py 同步到 TV main（D-082 五题升 version 2、D-083 容错解析）。
> §4：D-071 读反（docs/31 §5.1、docs/00 #4 改正，覆盖面定为只挂 commit_drafts、补录稿由 TV ① 覆盖）；docs/30 与 jev-comment-unit 未入库写明；505,000 字节出处；12% 与 2,419 篇的口径；docs/01 的调用次数；评论三列定为只落账本不回写；画像交集的单位；黑名单五份；金标准 8 处、c3 理由与 expect 对齐、报告头写金标准版本；47/50；v1_17 注释（TikHub、排在 v1_13 之后、ssll_sample id 带代际）；环境变量补全；33 条样本剥掉运营前缀并改正角色。`fq_shadow --from-db` 必须给 `--mappings`，按项目的 title_extraction 切；同篇同题多个 llm 模型取最新。测试 30 → 56。
> §5 三仓各开一个 PR（truth-vault、autowriter、sanshengliubu），链接见本仓这一轮的 PR 描述。
> **第四轮（PR #1 上 Codex 的七条评审）**：`/judge_draft` 的 `hard_rules` 指向没加载的层或不存在的题号 → 422（此前静默丢掉，调用方以为约束生效）；`write=true` 时 `subject_id` 留默认的 `draft` 或空白 → 422（否则所有草稿都写到同一主键、互相覆盖）；`banks` 重复出现同一层去重（此前重复名会把这层判成「未知」进 `ignored_banks`）；空的 `project_bank` / brief 编不出题 → 422；层间题号撞车（内联题库与 fq 同 id）→ 422 而不是后加载的盖掉先加载的。修改单：`DraftJudgement` 记下每条硬约束的 `want`，`repair_plan` 对 choice 题写「要改成『配置的目标选项』」并取该选项的定义（此前把 choice 题也当是非题翻转、定义取错边）。外部语料：搜索请求全部失败、或每篇笔记都处理失败时 `RunReport.systemic_failure` 置位，脚本打 `::error::` 并 exit 1（此前 TikHub 密钥失效也是绿的 run + 空产物）。测试 29 → 30，`test_api_judge_draft_with_brief` 与 `test_external_errors_and_seen_discipline` 各加了对应的红线断言。
