...
def fetch_from_db(limit: int, project: str | None):
    ...
    ans = _pg(url, key, f"note_feature_answers?select=subject_id,question_id,answer,evidence,invalid_reason"
                        f"&extractor=like.llm:*&run_tag=eq.primary&subject_id=in.({ids})")
    opus = defaultdict(dict)
    for a in ans:
        opus[a["subject_id"]][a["question_id"]] = {"answer": a["answer"], "evidence": a["evidence"], "invalid": a["invalid_reason"]}
    return notes, dict(opus)
...
def main():
    ...
    ap.add_argument("--title-extraction", default="markers")
    ...
    if args.from_raw:
        ...
        try:
            ...
            sys.exit(str(exc))
        def one(n):
            r = judge_note(client, bank, n["note_id"], n.get("raw_content") or "", title_extraction=args.title_extraction,
    ...
    if args.sql:
        ...
        for d in results:
            jr = JudgeResult(bank=d["bank"], bank_version=d["bank_version"], bank_sha256=d["bank_sha256"], model=d["model"],
                             subject_type=d["subject_type"], subject_id=d["subject_id"], items=d["items"])
