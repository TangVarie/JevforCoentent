...
"""外部语料 · 定时限额抓取（供应商 TikHub）。量级、频率、预算全在 config/external_corpus.yaml 里。
  ...
  python3 scripts/external_corpus.py --out docs/external-run.md --sql external.sql --notes-sql external_notes.sql
  python3 scripts/external_corpus.py --mock ...                    # 假供应商 + 假 Jev，跑通流程

产物：运行报告（md）、账本 SQL（note_feature_answers，subject_type = external_note，run_tag = external）、
外部笔记表 SQL（truth_vault.external_notes，migrations/notes_v1_18）、state/external_corpus_state.json（去重与月度计数）。
...
def main():
    ...
    ap.add_argument("--config", default=str(CFG)); ap.add_argument("--state", default=str(STATE))
    ...
    if args.notes_sql:
        Path(args.notes_sql).write_text(rows_to_sql_external(external_note_rows(rep.kept_notes, run_id), ), encoding="utf-8")
    if args.raw:
